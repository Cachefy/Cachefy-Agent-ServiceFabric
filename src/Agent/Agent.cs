        ...
        protected override IEnumerable<ServiceInstanceListener> CreateServiceInstanceListeners()
        {
            return new[]
            {
                new ServiceInstanceListener(serviceContext =>
                    ...
                    {
                        ServiceEventSource.Current.ServiceMessage(serviceContext, $"Starting Service on {url}");
                        ...
                        // Register AgentConfiguration from appsettings.json
                        builder.Services.Configure<AgentConfiguration>(
                            builder.Configuration.GetSection("AgentConfiguration"));

                        // Alternative: Register as singleton for direct injection
                        builder.Services.AddSingleton<AgentConfiguration>(provider =>
                        {
                            var config = new AgentConfiguration();
                            builder.Configuration.GetSection("AgentConfiguration").Bind(config);
                            return config;
                        });
